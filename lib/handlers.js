...
handlers._users.get = (data, callback) => {
  ...
  // @TODO Do this token verification to all methods as same
  let token = typeof(data.headers.token) == "string" ? data.headers.token : false;

  handlers._tokens.verification(token, phone, (tokenIsValid) => {
    ...
    if(phone){
      ...
      callback(403, {'Error':'Token not found in headers or token not valid'});
    }
...
handlers._tokens.verification = (id, phone, callback) => {
